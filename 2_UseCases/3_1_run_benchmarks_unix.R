...
plan(multiprocess)
gc()
mem <- mem %>% dplyr::bind_rows(tibble::tibble(pkg="nlrx",
                                               pos=1, 
                                               time=Sys.time(),
                                               mem=as.numeric(strsplit(system('free',intern=TRUE)[2], "\\s+")[[1]][4])/1024/1024))
...
library(RNetLogo)
gc()
mem <- mem %>% dplyr::bind_rows(tibble::tibble(pkg="RNetLogo",
                                               pos=1, 
                                               time=Sys.time(),
                                               mem=as.numeric(strsplit(system('free',intern=TRUE)[2], "\\s+")[[1]][4])/1024/1024))
