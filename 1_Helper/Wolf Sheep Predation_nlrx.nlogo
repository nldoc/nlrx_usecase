globals [ max-sheep fitness ]  ; don't let sheep population grow too large
; Sheep and wolves are both breeds of turtle.
breed [ sheep a-sheep ]  ; sheep is its own plural, so we use "a-sheep" as the singular.
breed [ wolves wolf ]
turtles-own [ energy age ]       ; both wolves and sheep have energy
...
to reproduce-sheep  ; sheep procedure
  if random-float 100 < sheep-reproduce [  ; throw "dice" to see if you will reproduce
    ...
    hatch 1 [ rt random-float 360 fd 1 set age 0 ]   ; hatch an offspring and move it forward 1 step
  ]
end
...
;; MODEL CHANGES:
; - added age variable
; - added line at the end of go to increase turtle age
; - deactivated simulation stops
; - added global variable fitness
; - added procedure to measure fitness on each tick
;; ADDED REPORTER:
to-report r.wolves
  report ifelse-value (any? wolves) [count wolves][0]
end

to-report r.wolves.prop
  report ifelse-value (any? turtles) [count wolves / (count wolves + count sheep)][0]
end

to-report r.wolves.age
  report ifelse-value (any? wolves) [mean [age] of wolves][0]
end

to-report r.sheep
  report ifelse-value (any? sheep) [count sheep][0]
end

to-report r.sheep.prop
  report ifelse-value (any? turtles) [count sheep / (count wolves + count sheep)][0]
end

to-report r.sheep.age
  report ifelse-value (any? sheep) [(mean [age] of sheep)][0]
end
...
to measure-fitness
  ...
  if (r.sheep < crit.sheep) [set fitness fitness + 1]
  if (r.wolves < crit.wolves) [set fitness fitness + 1]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot r.wolves.prop"
"pen-1" 1.0 0 -612749 true "" "plot r.sheep.prop"
...
"" ""
PENS
"pen-0" 1.0 0 -612749 true "" "plot r.sheep.age"
"pen-1" 1.0 0 -16777216 true "" "plot r.wolves.age"
